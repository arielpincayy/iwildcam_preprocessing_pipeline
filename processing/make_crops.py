...
def make_crops(json_file, input_folder, output_folder,
    ...
    target_w, target_h = calculate_median_size(data, input_folder, conf_threshold, accepted_categories)
    
    if target_w is None:
        print("No se encontraron detecciones válidas para calcular la mediana.")
        return
        ...
        try:
            with Image.open(img_path) as img:
                ...
                for i, det in enumerate(valid_dets):
                    ...
                    # Tamaño actual del bbox en px
                    curr_w = w_rel * img_w
                    curr_h = h_rel * img_h
                    
                    # Determinamos si es "Pequeño" (Menor que la mediana en área)
                    is_smaller = (curr_w * curr_h) < (target_w * target_h)
                    
                    crop = None
                    
                    if is_smaller:
                        # CASO 1: Bbox pequeño -> Expandimos el corte al tamaño target
                        left, top, right, bottom = get_expanded_crop_coords(
                            img_w, img_h, det['bbox'], target_w, target_h
                        )
                        crop = img.crop((left, top, right, bottom))
                        
                        # Si la imagen original era más pequeña que el target, 
                        # el crop resultante será menor al target. En ese caso extremo, reescalamos hacia arriba.
                        if crop.size != (target_w, target_h):
                            crop = crop.resize((target_w, target_h), Image.Resampling.LANCZOS)
                            
                    else:
                        # CASO 2: Bbox grande -> Recortamos ajustado y reescalamos hacia abajo
                        ...
                        crop = img.crop((left, top, right, bottom))
                        # Downsample al tamaño mediano
                        crop = crop.resize((target_w, target_h), Image.Resampling.LANCZOS)

                    # Guardar
                    base_name = os.path.basename(filename)
                    name_only, ext = os.path.splitext(base_name)
                    # Forzamos extensión jpg para evitar problemas de compatibilidad
